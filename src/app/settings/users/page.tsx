  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-0 sm:items-center sm:p-4" onClick={onClose}>
      <div className="relative w-full max-w-md rounded-t-2xl bg-white p-6 shadow-2xl sm:rounded-2xl" onClick={e => e.stopPropagation()}>
        ...
        <h2 className="text-xl font-semibold text-ink">ユーザーを追加</h2>
        <p className="mt-1 text-xs text-mist">メールアドレスと初期パスワードを指定して登録します。</p>

        {error && (
          <p className="mt-3 rounded-lg bg-alert/5 px-3 py-2 text-sm text-alert">{error}</p>
        )}

        <form onSubmit={handleSubmit} className="mt-4 space-y-3">
          <div>
            <label className="mb-1 block text-sm font-medium text-graphite">メールアドレス</label>
            <input
              type="email"
              required
              value={email}
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-graphite">表示名（任意）</label>
            <input
              type="text"
              value={displayName}
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-graphite">初期パスワード</label>
            <input
              type="text"
              required
              minLength={6}
              ...
              placeholder="6文字以上"
            />
            <p className="mt-1 text-[11px] text-mist">本人に共有してください。初回ログイン後に変更を推奨します。</p>
          </div>
          <div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="rounded-xl border border-line bg-white px-4 py-2 text-sm font-medium text-graphite transition hover:bg-bone"
            >
              キャンセル
            </button>
            <button
      ...
      <div className="space-y-6">
        <div>
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <h1 className="mt-3 text-3xl font-bold text-ink">ユーザー管理</h1>
        ...
        <div className="overflow-hidden rounded-3xl border border-line bg-white shadow-sm">
          {loading ? (
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="border-b border-[#e6dfcf] bg-bone text-left text-mist">
              ...
              <tbody>
                {users.map(u => {
                  ...
                  return (
                    <tr key={u.uid} className="border-b border-[#f0ebdf] text-ink">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <User2 size={14} className="text-mist" />
                          <span className="font-medium">{u.email || '(no email)'}</span>
                          {u.uid === user?.uid && (
                            <span className="rounded-full bg-[#eef3eb] px-2 py-0.5 text-[10px] text-matchaDeep">自分</span>
                          )}
                        </div>
                        <div className="mt-1 text-[11px] text-mist font-mono">{u.uid}</div>
                      </td>
                      <td className="px-4 py-3">
                        {u.role === 'admin' ? (
                          <span className="inline-flex items-center gap-1 rounded-full bg-bone px-2 py-0.5 text-xs font-medium text-matcha">
                            <ShieldCheck size={12} />
                            Admin
                          </span>
                        ) : (
                      ...
                      <td className="px-4 py-3 text-xs text-mist">{formatDate(u.createdAt)}</td>
                      <td className="px-4 py-3">
                        <div className="flex justify-end gap-2">
                          {dirty && (
                            <button
                              ...
                              className="inline-flex items-center gap-1 rounded-lg bg-ink px-3 py-1 text-xs font-medium text-paper transition hover:bg-[#205f43] disabled:opacity-60"
                            >
                              <Save size={12} />
                          ...
                          <button
                            ...
                            disabled={u.uid === user?.uid}
                            aria-label="削除"
                            className="rounded-lg p-1.5 text-alert transition hover:bg-alert/5 disabled:opacity-40 disabled:hover:bg-transparent"
                          >
                            <Trash2 size={14} />
                          </button>
                        </div>
        ...
        <div className="rounded-2xl border border-[#e6dfcf] bg-bone p-4 text-xs text-mist">

...
// Staff issues a delivery note (納品書) PDF — forward to the wholesale app (owns the PDF).
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const res = await fetch(`${WHOLESALE_BASE_URL}/api/wholesale/orders/${id}/delivery-note${qs ? `?${qs}` : ''}`, {
    ...
    return new NextResponse(buffer, {
      headers: {
        ...
        'Content-Disposition': res.headers.get('content-disposition') ?? 'inline; filename="delivery-note.pdf"',

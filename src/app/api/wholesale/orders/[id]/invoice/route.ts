...
// Staff issues an invoice (請求書) PDF — forward to the wholesale app (owns the PDF).
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const res = await fetch(`${WHOLESALE_BASE_URL}/api/wholesale/orders/${id}/invoice${qs ? `?${qs}` : ''}`, {
